...
We recommend you complete the _Custom Office Templates Vignette_ prior to walking through this template. This vignette focuses on how to construct workflows using templates. Here we will use the internal templates embedded in ``onbrand``.  If you want to copy them locally, modify the code snippet in the _Custom Office Templates Vignette_.
...
The list `b1` can then be added to the _content_ value of any placeholder with `list` as the type in the mapping file:
...
A fair number of R packages can save output to image files. Including a slide layout for images in your abstraction layer can be quite the useful feature / work-around.
...
- `"ph"` - text content to substitute for placeholders in the document placeholder content 
