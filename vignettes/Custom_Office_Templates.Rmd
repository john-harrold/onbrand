...
Sometimes you may wish to modify the underlying the `officer` object. When you `save_report()`, just before writing the file, the function will look for the field `post_processing`. Here you can define R code to modify the `officer` object. Within the environment this code is run the object `rpt` will be available. You should modify this object to apply any changes you want made. A value of `NULL` and this element will be ignored. 
...
Just like with PowerPoint templates you can define R code to post process your report as an `officer` object. You need to modify the object `rpt` defined in the post processing environment. If this element is missing or set to `NULL` it will be ignored. 
